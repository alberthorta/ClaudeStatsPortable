...
- **Screen sleep** (three tiers, see also `main.cpp::handleSleepModes`, `enterLightSleep`, `enterDeepSleep`):
  - **Tier 1 — backlight off**: holding BOOT 2 s calls `Display::setBacklight(false)`. Panel power (GPIO15) and the sprite stay alive so waking (any button press) is instant. Wake-up press is "consumed" (doesn't rotate / toggle view) via the `*WakeOnly` flags.
  - **Tier 2 — auto light sleep**: after `cfg.idleSleepMin` minutes of backlight-off with no interaction (`lastInteractionMs`), `enterLightSleep()` calls `esp_light_sleep_start()` with a 30 s timer and EXT1 wake on GPIO0/GPIO14 (ANY_LOW). The panel is pushed into SLPIN (`0x10`). Timer wake does a silent `doFetch` + `checkAutoOpen` and falls back into sleep; button wake marks interaction and returns to the loop where the normal handler detects the press edge. Disable tier 2 with `idleSleepMin = 0` in the web form.
  - **Tier 3 — deep sleep / drawer mode**: hold BOOT + KEY **simultaneously for 5 s** → arms an on-screen 5 s countdown (`Display::showDeepSleepArmed`). Any button press **after the combo is released** cancels (tracked via `comboReleasedOnce`). If the countdown reaches 0, `enterDeepSleep()` drops SLPIN, kills the TFT backlight and GPIO15 rail, schedules an RTC timer wake for the next auto-open (if enabled), and calls `esp_deep_sleep_start()`. On boot, `setup()` reads `esp_sleep_get_wakeup_cause()` first: `ESP_SLEEP_WAKEUP_TIMER` means we're here only to fire `Api::openWindow` and go right back to sleep (no screen). `ESP_SLEEP_WAKEUP_EXT1` is a normal boot triggered by a button.
  - The individual button handlers (BOOT short = rotate, BOOT 2 s = backlight off, KEY short = toggle view, KEY 10 s = wifi reset) are suppressed while `sleepMode != SleepMode::Normal` so the combo / armed countdown doesn't accidentally fire them.
...
- **Screen sleep — tier 1**: hold BOOT 2 s to turn off backlight; any button press wakes. Verify the wake press doesn't also trigger rotate/toggle-view.
- **Screen sleep — tier 2 (light)**: set `idleSleepMin` to 1 in the web form, turn off backlight via BOOT 2 s, wait 60 s. Serial prints `[sleep] light sleep`. Pressing a button wakes with `[sleep] light wake cause=3` (EXT1). Current draw should drop significantly (~1–10 mA) while asleep.
- **Screen sleep — tier 3 (drawer)**: hold BOOT + KEY together for 5 s; the screen shows "Entering deep sleep in 5…" counting down. Pressing any button after releasing the combo cancels. Let it reach 0 and the device deep sleeps (screen dark, current drops to ~0.5 mA). Press any button to cold-boot it back.
